...
Engine.prototype = {
  ...
  subscribe:function (clientId, channel, callback, context) {
    ...
    clientRedis.watch(self._ns + '/clients/' + clientId + '/exists', function (error) {
      clientRedis.get(self._ns + '/clients/' + clientId + '/exists', function(error, result) {
        if(result != 1) {
          self._server.debug('Will not subscribe for client ? to channel ?', clientId, channel);

          return;
        }

        var clientMulti = clientRedis.multi();

        clientMulti.sadd(self._ns + '/clients/' + clientId + '/channels', channel, function (error, added) {
          if (added === 1) self._server.trigger('subscribe', clientId, channel);
        });

        clientMulti.exec(function (err, results) {
          if(results === null) {
            self._server.debug('Client disappeared(1) while subscribing ? to channel ?', clientId, channel);
    ...
    function publish(message, channels, shardManager) {
      ...
      async.parallel(_.map(shardMap, function(shardInfo) {
        ...
        _.uniq(_.flatten(results)).forEach(function(clientId) {
          ...
          redis.watch(self._ns + '/clients/' + clientId + '/exists', function (error) {
            redis.get(self._ns + '/clients/' + clientId + '/exists', function(error, result) {
              if(result != 1) {
                self._server.debug('Will not publish for client ? - ? - to server ? ? ?', clientId, message, shard.shardName, error, result);
              ...
              multi.rpush(self._ns + '/clients/' + clientId + '/messages', jsonMessage);
              multi.publish(self._ns + '/' + clientId + '/notify', clientId);

              multi.exec(function (err, results) {
                if(results === null) {
                  self._server.debug('Client disappeared while publishing ? - ? - to server ?', clientId, message, shard.shardName);
