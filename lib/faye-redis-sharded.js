  ...
  function connectToShards(shards, isPrimary) {
    ...
    shards.forEach(function (shard) {
      ...
      var newShard = {
        ...
        lock: new Redlock([client], { retryCount: 0 }),
        lockWithRetry: new Redlock([client], { retryCount: 1000 }),
        redis: client,
      ...
      newShard.evalsha.add('subscribe', `
        ...
        if redis.call('GET', KEYS[3]) ~= 'subscribe' then
          return -2
        end
      ...
      newShard.evalsha.add('unsubscribe', `
        if redis.call('GET', KEYS[2]) ~= 'unsubscribe' then
          return -1
        end
...
Engine.prototype = {
  ...
  subscribe:function (clientId, channel, callback, context) {
    ...
    var lockKey = '/locks/' + clientId + '/channels' + channel;
    var lockTypeKey = '/locks/' + clientId + '/channels' + channel + '/lockType';

    clientRedis.set(lockTypeKey, 'subscribe');
    clientShard.lockWithRetry.lock(lockKey, 60000, function(err, lock) {
      if (!lock) {
        self._server.debug('Failed to subscribe client ? to channel ? (could not acquire lock)', clientId, channel);
      ...
      clientShard.evalsha.exec('subscribe', [
        ...
        if(results === -1) {
          ...
          self._server.debug('Will not subscribe for client ? to channel ? because lockType is incorrect', clientId, channel, err, results);
          lock.unlock();
        } else if(results === -3) {
  ...
  unsubscribe:function (clientId, channel, callback, context) {
    ...
    clientRedis.set(lockTypeKey, 'unsubscribe');
    clientShard.lockWithRetry.lock(lockKey, 60000, function(err, lock) {
      if (!lock) {
        self._server.debug('Failed to unsubscribe client ? from channel ? (could not acquire lock)', clientId, channel);

        return;
      }

      clientShard.evalsha.exec('unsubscribe', [
        self._ns + '/clients/' + clientId + '/channels',
        lockTypeKey,
      ], [
        channel
        ...
        if(results === -1) {
          self._server.debug('Will not unsubscribe for client ? from channel ? because lockType is incorrect', clientId, channel, err, results);
          lock.unlock();
        } else if(results === -2) {
          self._server.debug('Client ? is already unsubscribed from channel ?', clientId, channel, err, results);
          lock.unlock();
        } else {
  ...
  gc:function () {
    ...
    this._shardManagers[0].forEach(function (shard) {
      shard.lock.lock('/locks/gc', self.LOCK_TIMEOUT * 1000 + 1, function(err, lock) {
        if (!lock) {
          return;
        }

        var cutoff = new Date().getTime() - 1000 * 2 * timeout;
        var releaseLock = function() { lock.unlock(); };
