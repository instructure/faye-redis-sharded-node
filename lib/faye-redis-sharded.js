...
Engine.prototype = {
  ...
  clientExists:function (clientId, callback, context) {
    ...
    var cutoff = this._getClientTimeout();
  ...
  destroyClient:function (clientId, callback, context) {
    ...
    redis.zrem(self._ns + '/clients', clientId);
    redis.publish(shard.closeChannel, clientId);
    ...
    function publish(message, channels, shardManager) {
      ...
      async.parallel(_.map(shardMap, function(shardInfo) {
        ...
        _.uniq(_.flatten(results)).forEach(function(clientId) {
          ...
          var cutoff = self._getClientTimeout();

          redis.zscore(this._ns + '/clients', clientId, function (error, score) {
            // This client has expired between the subscriber collection and the actual publishing.
            if(parseInt(score, 10) <= cutoff) {
              self._server.debug('Will not queue for client ?: ? <= ?', clientId, score, cutoff);
  ...
  gc:function () {
    ...
    this._shardManagers[0].forEach(function (shard) {
      self._withLock('gc', shard.redis, function (releaseLock) {
        var cutoff = this._getClientTimeout(2),
          self = this;
  ...
  _getClientTimeout:function (multiplier) {
    multiplier = multiplier || 1.6;

    return new Date().getTime() - 1000 * multiplier * this._server.timeout;
  }
};
